...
- If no results found, you MUST suggest new terms OR try related collections
- Attempt #{attempt_node.attempt_num}: Think about alternative keywords, spellings, or related fields
...
{{
    ...
    "strategy": "what to try next - be specific"
}}"""
...
def search_with_retry_chain(
    ...
    while attempt_count < max_attempts:
        ...
        should_continue, next_terms, decision = evaluate_and_decide_next(
            new_node, user_query, firebase_schema, max_attempts
        )
...
def generate_final_response(
    ...
    # Extract sample data for LLM context on what data exists
    sample_data = {}
    for col, docs in firebase_data.items():
        if docs:
            sample_doc = docs[0].copy()
